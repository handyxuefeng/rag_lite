...
# 必须搭配 Logger 实例、日志级别、格式化器（Formatter）等一起使用，
# 且只有调用 logger.debug()/info()/error() 等方法触发日志时，才会写入文件。
...
class LoggerManager:
    ...
    def _initialize(self):
        ...
        if self.enable_file:
            ...
            # mkdir创建目录，parents=True  如果父目录不存在，则自动创建所有的父目录
            # exist_ok = True 如果目录存在，不抛出错误
